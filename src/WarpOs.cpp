...
int main(int argc, char *argv[])
{
	...
	if(vm.count("inproj"))
	{
		...
		if (inproj == "mercator") {projType = PolyProjectArgs::Mercator;}
		if (inproj == "cassini") {projType = PolyProjectArgs::Cassini;}
		if (inproj == "bonnes") {projType = PolyProjectArgs::BonneS;}
		if (inproj == "bonnei") {projType = PolyProjectArgs::BonneI;}
		if (inproj == "osi") {projType = PolyProjectArgs::OSI;}
	}

	if(inputImageFilename.length() == 0) {cout << desc << endl; exit(0);}

...
{
	if (num < 0 || (unsigned int)num >= vals.size())
		ThrowError<invalid_argument>("Invalid value number", __LINE__, __FILE__);
	return vals[num];
}
...
void DelimitedFileLine::ProcessLine(const char *line, string &delimiter)
{
	if (delimiter.size() == 0)
	{
		ThrowError<invalid_argument>("Delimiter not set", __LINE__, __FILE__);
	}
	// cout << line << endl;
	// cout << "search ";
	int pos = 0;
	...
	while (del != NULL)
	{
		...
		if (pos - 2 >= 0 && line[pos - 2] == '\\')
		{
			delimitedComma = 1;
		} // Ignore delimited commas
...
{
	if (num < 0 || num >= (int)NumLines())
		ThrowError<invalid_argument>("Invalid line number", __LINE__, __FILE__);
	return lines[num];
}
